...
class CalculBio extends Controller
{
    public function calcularBiorritme(Request $request) {
        ...
        try {
            ...
            $dataNaixement = \date('Y-m-d', \strtotime($dataNaixement));
            $dataNaixementTransformada = \date('d/m/Y', \strtotime($dataNaixement));

            return \view("bio.result", ["nomUsuari" => $nomUsuari, "dataNaixement" => $dataNaixementTransformada, "resultatFisic" => $resultatFisic, 
                                        ...
                                        "dataActual" => $dataActual, "dataNaixementSenseTransformar" => $dataNaixement]);
            }
        catch (\Exception $e) {
    ...
    public function calcularBiorritmeDiaEspecific(Request $request) {
        ...
        try {
            ...
            $nomUsuari = $request->input("nomusuari");
            $dataNaixement = $request->input("datanaixement");
            $dataDesitjada = $request->input("datadesitjada");

            // Revisa si les dades enviades
            if (strlen($nomUsuari) == 0 || strlen($dataNaixement) == 0 || strlen($dataDesitjada) == 0) {
                throw new \Exception("Error dades incompletes");
            }

            // La data de naixement es passa a tipus DateTime, i també es recupera la data actual
            $dataInicial = new DateTime($dataNaixement);
            $dataActual = new DateTime($dataDesitjada);
            ...
            // S'omplen els arrays amb la informacio des de fa 15 dies fins a 15 dies futurs partint del dia actual
            for ($index = -15; $index <= 15; $index++) {
                $resultatEixX = \date("d-m-Y", \strtotime(\date('Y/m/d', \strtotime($dataDesitjada)) . "+" . $index . " days"));
                $resultatFisicX = \sin((($diesDiferencia + $index) / 23) * 2 * \pi());
                $resultatEmotiuX = \sin((($diesDiferencia + $index) / 28) * 2 * \pi());
                $resultatIntelectualX = \sin((($diesDiferencia + $index) / 33) * 2 * \pi());
            ...
            // Es converteixen les dates a format espanyol
            $dataActual = \date('d/m/Y', \strtotime($dataDesitjada));
            $dataNaixement = \date('Y-m-d', \strtotime($dataNaixement));
            $dataNaixementTransformada = \date('d/m/Y', \strtotime($dataNaixement));

            return \view("bio.resultdiaespecific", ["nomUsuari" => $nomUsuari, "dataNaixement" => $dataNaixementTransformada, "resultatFisic" => $resultatFisic, 
                                        "resultatEmotiu" => $resultatEmotiu, "resultatIntelectual" => $resultatIntelectual,
                                        "resultatsEixX" => $resultatsEixX, "resultatsFisicEixY" => $resultatsFisicEixY,
                                        "resultatsEmotiuEixY" => $resultatsEmotiuEixY, "resultatsIntelectualEixY" => $resultatsIntelectualEixY,
                                        "dataActual" => $dataActual, "dataNaixementSenseTransformar" => $dataNaixement]);
